...
const parts = await Promise.all(['prefix.js','ui.js','art.js','timer.js','boot.js'].map(read));
const app = parts.join('\n');
const [template, style, rules] = await Promise.all(['template.html','styles.css','game.js'].map(read));
