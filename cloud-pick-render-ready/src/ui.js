  ...
  function renderHUD(){
    canvas.setAttribute('aria-label', '구름 보드. ' + G.REWARD_CELLS.map(cell => `${CELL_NAMES[cell]} ${state.board[cell]}코인`).join(', ') + '. 번호가 표시된 세 구름 또는 아래 선택 버튼으로 조작하세요.');
  ...
  function lessonCopy(){
    ...
    if(tutor.step===2)return done?{title:'같은 생각. 둘 다 빈손.',body:'친구도 가운데를 골랐네. 같은 구름을 고른 둘 모두 이번에는 0코인이야.',note:'이미 모은 2코인은 잃지 않아.'}:{title:'겹치면, 둘 다 빈손.',body:'이번 연습의 가운데는 3코인이야. 2번 구름을 골라봐.',note:'가운데 구름은 네 명 모두 고를 수 있어.'};
