  ...
  function renderChoiceBar() {
    ...
    if(state.phase==='choose'){
      if(p.locked){ $('choiceBar').innerHTML=`<div class="choice-intro"><strong>선택 확정됨</strong><small>공개 전까지 선택을 바꿀 수 없습니다.</small></div><div class="selected-ticket">✓ ${selected===null?'선택 없음':CELL_NAMES[selected]+' 구름'}<small>공개 대기</small></div><button class="lock-btn" disabled>확정 완료</button>`;return; }
  ...
  function showHistory(){showDialog(`<h2>내 라운드 기록</h2>${historyBody()}${liveNote()}<button class="text-button setup-secondary" data-action="closeDialog">돌아가기</button>`);}
  ...
  function lessonCopy() {
    ...
    if(tutor.step===3)return done
      ? {title:'선택 확정됨',body:'모두 확정하거나 시간이 끝나면 함께 공개합니다.',note:'공개 전에는 다른 사람의 선택과 확정 여부가 보이지 않습니다.'}
      : tutor.changed
