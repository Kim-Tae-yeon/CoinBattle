  ...
  let matchInfo = { enabled: false, fallbackMs: 20000, rounds: 10, seconds: 10 };
  ...
  function randomSetupHTML() {
    ...
    return `<section class="setup-panel"><button class="back-button" data-action="backHome">← 처음으로</button><h1 id="screenHeading" class="screen-title" tabindex="-1">${random ? '랜덤 매칭' : online ? '친구와 플레이' : '봇 연습'}</h1><p class="panel-desc">${random ? '상대를 찾으면 자동으로 입장합니다.' : online ? '친구에게만 초대 링크를 보내세요.' : '나와 봇 3명. 연습 점수는 따로 쌓이지 않아요.'}</p>${nameHTML()}<div class="match-start">${content}</div><p id="setupError" class="inline-error" role="alert">${escapeHTML(ux.error)}</p>${choices}</section>`;
  }
  function queueHTML() {
    const q = queue?.view || {}, found = q.status === 'ready', count = q.count || 1;
    return `<section class="queue-panel"><h1 id="screenHeading" class="screen-title" tabindex="-1">${found ? '매칭 완료' : '상대 찾는 중'}</h1><p class="panel-desc">${found ? `사람 ${count}명${q.bots ? ` + 봇 ${q.bots}명` : ''}. 접속 확인 중입니다.` : '4명이 모이면 자동 입장합니다.'}</p><div class="queue-card"><div class="queue-card-top"><span>${found ? '매칭 준비' : '현재 대기 인원'}</span><strong id="queueCount" aria-live="polite">${count}<small> / 4</small></strong></div><div class="queue-seats" aria-label="사람 ${count}명${found && q.bots ? `, 봇 ${q.bots}명` : ''}">${[0,1,2,3].map(i => `<div class="queue-seat ${i < count ? 'occupied' : found ? 'bot-seat' : 'empty'}">${i === 0 ? avatar(0) : `<span class="unknown-seat" aria-hidden="true">${i < count ? '☾' : found ? '◇' : '·'}</span>`}<small>${i === 0 ? '나' : i < count ? '다른 사람' : found ? '봇' : '빈자리'}</small></div>`).join('')}</div><div class="queue-clock"><span>내 대기 시간</span><b id="queueElapsed" aria-live="off">00:00</b></div></div><p class="queue-status" id="queueStatus" role="status">${escapeHTML(queue?.error || q.notice || (found ? '접속을 확인하는 중…' : '상대 찾는 중'))}</p><p class="queue-hint" id="queueHint"></p><button class="btn btn-line" data-action="cancelMatch" ${queueCancelling ? 'disabled' : ''}>${queueCancelling ? '취소를 확인하는 중…' : '매칭 취소'}</button><p class="queue-fine">${found ? '준비 완료 후 게임이 시작됩니다.' : '1명일 때는 다른 사람을 기다립니다.'}</p></section>`;
  }
  ...
  function readQueueReply(reply, epoch) {
    ...
    if (!previous || previous.status!==reply.status || previous.count!==reply.count || previous.notice!==reply.notice) {render();if(previous?.status!==reply.status && reply.status==='ready')announce('매칭 완료. 연결 확인 중입니다.');}
    else if ($('queueStatus')) $('queueStatus').textContent=reply.notice || (reply.status==='ready'?'접속을 확인하는 중…':'상대 찾는 중');
    updateQueueClock();
  }
