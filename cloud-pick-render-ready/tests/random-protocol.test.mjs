...
test('public matchmaking over real HTTP and SSE', { timeout: 75000 }, async t => {
  ...
  try {
    ...
    await t.test('health advertises public-match capability and fixed rules',async()=>{const h=await json('/api/health');assert.equal(h.matchmaking.enabled,true);assert.equal(h.matchmaking.minimumHumans,2);assert.equal(h.matchmaking.rounds,5);});
    ...
    await t.test('four unrelated join requests create exactly one public room',async()=>{await until(()=>clients.every(c=>c.latest.players.length===4));assert.equal(new Set(matches.map(v=>v.code)).size,1);for(const c of clients){assert.equal(c.latest.matchmaking.humans,4);assert.equal(c.latest.matchmaking.bots,0);assert.equal(c.latest.config.rounds,5);assert.equal(c.latest.config.seconds,10);assert.ok(c.latest.players.every(p=>!p.bot));}});
