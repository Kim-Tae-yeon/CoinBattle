...
test('public matchmaking over real HTTP and SSE', { timeout: 190000 }, async t => {
  ...
  try {
    ...
    await t.test('all ten rounds finish with identical correct totals',async()=>{await until(()=>clients.every(c=>c.latest.phase==='finished'));for(const c of clients){assert.deepEqual(c.latest.players.map(p=>p.score),clients[0].latest.players.map(p=>p.score));assert.ok(c.latest.history.every(h=>h.results.length===1&&h.results[0].id===c.id));assert.equal(c.latest.players.find(p=>p.id===c.id).score,c.latest.history.reduce((s,h)=>s+h.results[0].gain,0));}});
