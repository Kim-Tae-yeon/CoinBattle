...
test('public matchmaking over real HTTP and SSE', { timeout: 160000 }, async t => {
  ...
  try {
    ...
    for(let round=2;round<=10;round++){
      ...
      for(const c of clients){
        assert.ok(c.latest.results.every(r=>r.gain===c.latest.board[r.cell]));
        assert.deepEqual(c.latest.board.filter(Boolean).sort((a,b)=>a-b), round>=3?[1,2,3,5,6]:[1,1,2,2,4]);
        assert.equal('jackpotBag' in c.latest,false);
      }
    }
