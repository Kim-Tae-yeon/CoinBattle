  ...
  function setupHTML() {
    ...
    return `<section class="setup-panel"><button class="back-button" data-action="backHome">← 처음으로</button><div class="eyebrow">PICK YOUR COMPANY.</div><h1 id="screenHeading" class="screen-title" tabindex="-1">오늘 밤, 누구와?</h1><p class="panel-desc">가볍게 연습하거나, 낯선 상대의 속마음을 읽거나.</p>${nameHTML()}<div class="mode-picks" role="group" aria-label="플레이 방식"><button class="mode-card" data-action="kindSolo" aria-pressed="${!online}"><strong>봇과 연습</strong><small>나 + 봇 3명</small></button><button class="mode-card" data-action="kindFriends" aria-pressed="${online}"><strong>온라인 플레이</strong><small>랜덤 상대 또는 친구</small></button></div>${online ? `${!serverAvailable ? `<div class="server-offline">${!backendChecked ? '멀티플레이 서버를 확인하고 있어요…' : '지금은 혼자 연습만 가능해요. 멀티플레이는 서버에 연결된 주소에서 열어 주세요.'}</div>` : ''}<button class="btn btn-primary" data-action="matchmake" ${busy || !serverAvailable ? 'disabled' : ''}>${busy ? '상대를 찾는 중…' : '랜덤매칭 시작 →'}</button><p class="inline-note matchmaking-note">다른 플레이어 1명과 매칭되면 봇 2명이 합류해 바로 시작해요.</p><div class="friend-divider"><span>친구와 함께하려면</span></div><button class="btn btn-line" data-action="create" ${busy || !serverAvailable ? 'disabled' : ''}>비공개 방 만들기</button><button class="text-button setup-secondary" data-action="joinDialog" ${busy || !serverAvailable ? 'disabled' : ''}>초대 코드로 참가하기</button>${!serverAvailable && backendChecked ? '<button class="text-button setup-secondary" data-action="connectionHelp">연결 방법 보기 →</button>' : ''}` : `<div class="setup-meta"><span>${config.rounds}라운드 · 선택 ${config.seconds}초</span><button class="text-button" data-action="settings">규칙 변경</button></div><button class="btn btn-primary" data-action="solo" ${busy ? 'disabled' : ''}>연습 시작 →</button>`}<p id="setupError" class="inline-error" role="alert">${escapeHTML(ux.error)}</p>${storage.get(LEARN_KEY) !== 'done' ? '<button class="text-button setup-secondary" data-action="tutorial">조작이 낯설다면, 먼저 배워보기 →</button>' : ''}</section>`;
  }
  ...
  function lobbyHTML() {
    ...
    if(session?.matchmaking)return `<section class="matchmaking-wait"><div class="eyebrow">FINDING A STRANGER.</div><div class="matchmaking-orbit" aria-hidden="true">${avatar(0)}<i></i><span>?</span></div><h1 id="screenHeading" class="screen-title" tabindex="-1">수상한 상대를<br>찾고 있어요.</h1><p>매칭되면 봇 두 명이 합류해<br>5라운드 게임이 바로 시작돼요.</p><button class="btn btn-line" data-action="leave">매칭 취소</button></section>`;
  ...
  function applyState(next){
    ...
    if(session?.matchmaking&&next.phase!=='lobby')session.matchmaking=false;
  ...
  function connectSession(result){
    closeDialog();disconnect();localRoom=null;tutor=null;tutorSerial++;mode='online';pausedAt=0;session={token:result.token,id:result.id,code:result.code,matchmaking:!!result.matchmaking};storage.set('cloud-pick:session',JSON.stringify(session),true);connection='connecting';optimisticCell=null;
  ...
  async function matchmake(){
    if(!onlineRequired()||busy)return;readName();busy=true;ux.error='';render();
    try{connectSession(await api('/api/matchmake',{name},null));toast('랜덤 상대를 찾고 있어요.');}
    catch(e){ux.error=e.message||'매칭을 시작하지 못했어요.';}
    finally{busy=false;render();}
  }
  ...
  document.addEventListener('click',event=>{
    ...
    else if(a==='solo')startSolo();else if(a==='matchmake')matchmake();else if(a==='create')createRoom();else if(a==='joinDialog')joinDialog();
