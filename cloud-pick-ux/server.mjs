...
try {
  ...
  // Some containers disallow enumerating host network interfaces. This is only
  // used for the optional local-network hint, so the web server can still run.
}
  ...
  try {
    ...
    if (url.pathname.startsWith('/api/') && req.method === 'POST') {
      ...
      if (url.pathname === '/api/matchmake') {
        let room = matchmakingRoomCode ? rooms.get(matchmakingRoomCode) : null;
        const waitingHumans = room?.players.filter(p => !p.bot && p.connected && !p.departed) || [];
        if (!room || room.phase !== 'lobby' || waitingHumans.length !== 1) {
          if (rooms.size >= MAX_ROOMS) throw new Error('지금은 방이 너무 많아요. 잠시 뒤 다시 시도해 주세요.');
          let c; do { c = code(); } while (rooms.has(c));
          room = new Room(c, { rounds: 5, seconds: 10, fillBots: true });
          rooms.set(c, room); matchmakingRoomCode = c;
          const result = issueSession(room, data.name); result.matchmaking = true;
          return json(res, 200, result);
        }
        const result = issueSession(room, data.name);
        matchmakingRoomCode = null;
        room.start(room.hostId);
        result.state = room.snapshot(result.id); result.matchmaking = true;
        broadcast(room);
        return json(res, 200, result);
      }
...
const tick = setInterval(() => {
  ...
  for (const room of rooms.values()) {
    ...
    if (!active && now - room.touchedAt > 5 * 60 * 1000 || now - room.createdAt > 24 * 60 * 60 * 1000) {
      if (matchmakingRoomCode === room.code) matchmakingRoomCode = null;
